...
in a pattern that synthetic controls cannot replicate. This does not
answer the question --- the observed pattern is consistent with either
absence of a single \(Q\) or the inadequacy of the model class used ---
but it establishes that the question has empirical content.
...
\subsection{2.4 Consistency Is Automatic --- But Only in Discrete
Time}\label{consistency-is-automatic-but-only-in-discrete-time}
...
published by the European Central Bank (ECB) over the period January
2014 to December 2024 (2,805 trading days). The dataset comprises 12
maturities: 3 months, 6 months, and 1, 2, 3, 5, 7, 10, 15, 20, 25, and
30 years. We convert par yields to zero-coupon yields using standard
bootstrap methods. We note that the ECB par yield curves are
model-fitted (Svensson parametrization), not raw bond prices; this
introduces a layer of interpolation and smoothing that may affect
cross-segment calibration results.
...
amplify. Synthetic controls confirm this pattern is not a generic
